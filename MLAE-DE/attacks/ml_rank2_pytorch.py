...
class MLRank2(object):
    ...
    def generate_np(self, x, **kwargs):
        ...
        num_instances = y_target.shape[0]
        ...
        A_pos_t = torch.tensor(A_pos)
        A_neg_t = torch.tensor(A_neg)
        B_pos_t = torch.tensor(B_pos)
        B_neg_t = torch.tensor(B_neg)
        ...
        if torch.cuda.is_available():
            ...
            A_pos_t = A_pos_t.cuda()
            A_neg_t = A_neg_t.cuda()
            B_pos_t = B_pos_t.cuda()
            B_neg_t = B_neg_t.cuda()
        ...
        for outer_step in range(binary_search_steps):
            ...
            batch_A_pos = A_pos[:batch_size]
            batch_A_neg = A_neg[:batch_size]
            batch_B_pos = B_pos[:batch_size]
            batch_B_neg = B_neg[:batch_size]
            ...
            for iteration in range(max_iter):
                ...
                output, loss, l2dist, newimg = criterion(self.model, y_target_t, modifier, x_t, self.clip_max, self.clip_min, const_t, A_pos_t, A_neg_t, B_pos_t, B_neg_t, num_instances)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def criterion(model, y, modifier, x_t, clip_max, clip_min, const, A_pos, A_neg, B_pos, B_neg, num_instances):
    ...
    #print(A_pos)
    #print(A_pos.shape)
    #print(output[A_pos==1])
    #print(output[A_pos==1].shape)
    #A_pos_v = output[A_pos==1].view(num_instances, -1)
    A_pos_v = output * A_pos
    #A_neg_v = output[A_neg==1].view(num_instances, -1)
    #B_pos_v = output * B_pos
    B_neg_v = output * B_neg
    #print(B_neg_v.shape)
    B_neg_v[B_neg_v==0] = 1

    max_A_pos = torch.max(A_pos_v, 1).values
    min_B_neg = torch.min(B_neg_v, 1).values

    #max_B_pos = torch.max(B_pos_v, 1).values
    #min_A_neg = torch.min(A_neg_v, 1).values
    loss3 = torch.max(torch.zeros_like(max_A_pos), max_A_pos - min_B_neg)
            #+ torch.max(torch.zeros_like(max_B_pos), max_B_pos - min_A_neg)

    loss2 = torch.sum(l2dist)
    loss1 = torch.sum(const * (loss1 + loss3))
    loss = loss1 + loss2
